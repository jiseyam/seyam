...
const progressFill = preloader.querySelector<HTMLElement>('#progress-fill');
const progressText = preloader.querySelector<HTMLElement>('#progress-text');
...
// ─────────────────────────────────────────────
function setupFadeUpObserver(): void {
...
async function boot(): Promise<void> {
  ...
  const progressStatus = preloader.querySelector<HTMLElement>('#progress-status');
  frames = await preloadFrames((pct) => {
    if (progressFill) progressFill.style.width = `${pct}%`;
    if (progressText) progressText.textContent = `${pct}%`;
    if (progressStatus) {
      if (pct < 30) progressStatus.textContent = 'Initializing 3D Character Canvas...';
      else if (pct < 70) progressStatus.textContent = 'Loading Glass Components & Shaders...';
      else if (pct < 99) progressStatus.textContent = 'Finalizing Smooth Physics Engine...';
      else progressStatus.textContent = 'Welcome to Seyam\'s Portfolio 🚀';
    }
  });
  ...
  if (typewriterEl) {
    setupTypewriter(typewriterEl as HTMLElement, heroRoles, { speed: 80, pause: 2200 });
  }
