 ...
 */
export function setupFadeInOnScroll(selector: string, options: AnimationOptions = {}): void {
  const {
    threshold = 0.15,
...
export function animateSkillBars(): void {
  const bars = document.querySelectorAll<HTMLElement>('.skill-fill[data-level]');
 ...
 */
export function setupMagneticButtons(selector: string = '.magnetic'): void {
...
export function setupTypewriter(
  el: HTMLElement,
  texts: string[],
  options: { speed?: number; pause?: number } = {}
): void {
...
export function setupScrollSpy(
  ...
  const links = document.querySelectorAll<HTMLAnchorElement>(navLinkSelector);
    ...
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const active = document.querySelector<HTMLAnchorElement>(
          `${navLinkSelector}[href="#${entry.target.id}"]`
        );
