...
export function createContact(): HTMLElement {
  ...
  const statusEl = section.querySelector<HTMLElement>('#form-status');

  form?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const btn = form.querySelector<HTMLButtonElement>('#contact-submit-btn');
    const btnText = form.querySelector<HTMLSpanElement>('.btn-text');
