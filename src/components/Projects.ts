...
function renderProjectsSection(section: HTMLElement, projectsList: Project[]): void {
...
function renderProjectCard(p: Project, index: number): string {
...
function initShowcaseEngine(section: HTMLElement, projectsList: Project[]): void {
  let currentIndex = 0;
  let isAnimating = false;

  const counterCurr = section.querySelector<HTMLElement>('#counter-curr')!;
  const counterTotal = section.querySelector<HTMLElement>('#counter-total')!;
  const railContainer = section.querySelector<HTMLElement>('#proj-deck-rail')!;
  const emptyCard = section.querySelector<HTMLElement>('#empty-projects-card');

  function getCards(): HTMLElement[] {
    return Array.from(section.querySelectorAll<HTMLElement>('.proj-showcase-card'));
  }
  ...
  function goToIndex(targetIndex: number, direction: 'next' | 'prev' = 'next'): void {
    const visibleCards = getVisibleCards();
    if (visibleCards.length === 0 || isAnimating) return;
  ...
  function updateUI(): void {
    ...
    if (counterCurr) counterCurr.textContent = String(currentIndex + 1).padStart(2, '0');
    if (counterTotal) counterTotal.textContent = String(total).padStart(2, '0');

    const railItems = section.querySelectorAll<HTMLElement>('.proj-rail-item');
  ...
  railContainer?.addEventListener('click', (e) => {
    const item = (e.target as HTMLElement).closest('.proj-rail-item') as HTMLElement;
  ...
  let lastWheelTime = 0;
  const showcaseWrap = section.querySelector<HTMLElement>('#proj-showcase')!;

  showcaseWrap?.addEventListener('wheel', (e: WheelEvent) => {
    const now = Date.now();
  ...
  let touchStartY = 0;
  showcaseWrap?.addEventListener('touchstart', (e: TouchEvent) => {
    touchStartY = e.touches[0].clientY;
  }, { passive: true });

  showcaseWrap?.addEventListener('touchend', (e: TouchEvent) => {
  ...
  section.querySelectorAll<HTMLButtonElement>('.filter-btn').forEach(btn => {
  ...
  section.querySelectorAll<HTMLElement>('.card-modal-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const id = btn.dataset.id;
      const project = projectsList.find(p => p.id === id);
      if (project) openModal(project, section);
    });
  });
...
function openModal(project: Project, section: HTMLElement): void {
  const overlay = section.querySelector<HTMLElement>('#project-modal');
  const content = section.querySelector<HTMLElement>('#modal-content');
...
function closeModal(section: HTMLElement): void {
